...
def test_return_period_maps_matches_reopened_surface_store(tmp_path):
    trackset = multi_trackset()
    ...
    reopened = WindFootprintSet.open(store.path)
    persisted_maps = return_period_maps(reopened, [3])
    in_memory = WindFootprintSet(
        dataset=xr.Dataset(
            {
                "max_wind_speed_ms": (
                    ("event", "lat", "lon"), np.array([[[10.0]], [[20.0]]])
                ),
...
def test_return_period_map_set_writes_north_up_geotiffs_and_overwrites(tmp_path):
    maps = ReturnPeriodMapSet(
        data=xr.DataArray(
            ...
            coords={"return_period": [10.0], "lat": [10.0, 11.0], "lon": [120.0, 121.0]},
